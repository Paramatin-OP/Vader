...
async def yardim(event):
    ...
    if tgbotusername is not None or hell_input == "text":
        results = await event.client.inline_query(tgbotusername, "@Hitler_X_bot")
...
async def info(event):
    ...
    if input_str == "text":
        string = (
            "Total {count} commands found in {plugincount} sudo plugins of BlackUserẞø†\n\n"
        )
        hellcount = 0
        plugincount = 0
        ...
        if len(string) > 4095:
            ...
            reply_text = f"All commands of the Blackuserẞø† are [here]({url})"
